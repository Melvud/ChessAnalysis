...
class LocalGameAnalyzer(
    ...
    /**
     * ✅ КРИТИЧЕСКИ ВАЖНО: Анализ партии с нормализацией оценок
     *
     * Эта функция анализирует всю партию и возвращает FullReport, где:
     * - ВСЕ оценки (cp, mate) нормализованы к точке зрения БЕЛЫХ
     * - Положительная оценка (+) = белые выигрывают
     * - Отрицательная оценка (-) = черные выигрывают
     * - Классификация ходов (MoveClass) правильно распределена по игрокам
     *
     * Нормализация происходит автоматически для локального движка.
     * Серверный движок уже возвращает нормализованные данные.
     */
    ...
    /**
     * ✅ КРИТИЧЕСКИ ВАЖНО: Нормализация оценок к точке зрения белых
     *
     ...
     *
     * После нормализации ВСЕ оценки в FullReport показываются с точки зрения БЕЛЫХ:
     * - Положительная оценка (+) = белые выигрывают
     * - Отрицательная оценка (-) = черные выигрывают
     * - Нулевая оценка (0) = равная позиция
     *
     * Эта функция используется ТОЛЬКО для локального движка.
     * Серверный движок уже возвращает нормализованные оценки.
     */
    private fun normalizeToWhitePOV(
        ...
        val linesEval = pos.lines.map { line ->
            // Нормализуем cp к точке зрения белых
            val cp = line.cp?.let { if (!whiteToPlay) -it else it }

            // Нормализуем мат к точке зрения белых
            val mate = line.mate?.let { m ->
                when {
        ...
        // Логируем нормализацию для первых нескольких позиций (для отладки)
        if (idx < 5) {
            Log.d(TAG, "🔄 Normalized pos[$idx]: whiteToPlay=$whiteToPlay, " +
                    "raw_cp=${pos.lines.firstOrNull()?.cp}, normalized_cp=${linesEval.firstOrNull()?.cp}")
        }
