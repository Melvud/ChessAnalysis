...
object EngineClient {
    ...
    private fun parseClockDataFromPgn(pgn: String): ClockData {
        Log.d(TAG, "parseClockDataFromPgn: starting parse...")

        // Более гибкий паттерн для парсинга часов
        ...
        // Убираем заголовки и берём только ходы
        val movesSection = pgn.substringAfter("\n\n").trim()

        // Паттерн для поиска ходов с часами: "1. e4 {[%clk ...]} e5 {[%clk ...]}"
        val movePattern = Regex("""(\d+)\.\s*(\S+)(?:\s*\{[^}]*\[%clk\s+[^\]]+\][^}]*\})?(?:\s+(\S+)(?:\s*\{[^}]*\[%clk\s+[^\]]+\][^}]*\})?)?""")
        ...
        clockPattern.findAll(pgn).forEach { match ->
            ...
            if (plyIndex % 2 == 0) {
                whiteTimes.add(centiseconds)
                Log.d(TAG, "White move ${plyIndex / 2 + 1}: ${formatClock(centiseconds)}")
            } else {
                blackTimes.add(centiseconds)
                Log.d(TAG, "Black move ${plyIndex / 2 + 1}: ${formatClock(centiseconds)}")
            }
            plyIndex++
        }

        Log.d(TAG, "parseClockDataFromPgn: parsed ${whiteTimes.size} white clocks, ${blackTimes.size} black clocks")
    ...
    suspend fun analyzeMoveRealtimeDetailed(
        ...
        client.newCall(req).execute().use { resp ->
            ...
            // После хода ходит ПРОТИВОПОЛОЖНАЯ сторона от той, что сделала ход
            ...
            val evalAfter: Float = when {
                top?.mate != null -> {
                    // Stockfish: положительный mate = выигрывает сторона, которая ходит
                    // Нужно: положительный = выигрывают белые
                    val normalizedMate = when {
                        top.mate!! == 0 && whiteToPlayAfter -> -1  // Белые заматованы (ход белых, но они в мате)
                        top.mate!! == 0 && !whiteToPlayAfter -> 1  // Чёрные заматованы (ход чёрных, но они в мате)
                        whiteToPlayAfter -> top.mate!!  // Ход белых: положительное mate = белые выигрывают
                        else -> -top.mate!!  // Ход чёрных: положительное mate = чёрные выигрывают → инвертируем
                    }
                    if (normalizedMate > 0) 30f else -30f
                }
                top?.cp != null -> {
                    // Stockfish: положительный cp = преимущество стороны, которая ходит
                    // Нужно: положительный cp = преимущество белых
            ...
            val linesAfter = parsed.lines.map { line ->
                ...
                val normalizedMate = line.mate?.let { m ->
                    when {
                        ...
                        whiteToPlayAfter -> m  // Ход белых: без изменений
